...
def send_artnet_packet(data, universe, ip='127.0.0.1', port=6454):
    ...
    artnet_packet = header + opcode + protocol_version + sequence + physical + subuni + net + length + bytearray(data)
    sock.sendto(artnet_packet, (ip, port))
    sock.close()

def main():
    num_fixtures = 27  # Adjust this value to match the number of fixtures
    universe = 1
    shift_step = 1
    shift = 1
    delay = 0.1
